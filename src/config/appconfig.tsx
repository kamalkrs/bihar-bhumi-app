export const Appconfig = {
    API_URL: 'https://services.biharbhumiseva.in/api/call/',
    UPLOAD_API_URL: 'https://services.biharbhumiseva.in/api/uploads/',
    UPLOAD_PATH: 'https://www.biharbhumiseva.in/assets/uploads/',
    USER_CUSTOMER: 1,
    USER_LAND_OWNER: 2,
    USER_BROKER: 3,
    USER_MUNSI: 4,
    USER_AMIN: 5,
    USER_CO: 6,
    USER_SDO: 7,
    USER_BHUMI_LOCKER: 8,
    Url_Privacy_Policy: 'https://www.biharbhumiseva.in/privacy-policy.php',
    Url_Terms_Conditions: 'https://www.biharbhumiseva.in/terms-conditions.php',
    Url_Contact_Us: 'https://www.biharbhumiseva.in/contact-us.php',
}